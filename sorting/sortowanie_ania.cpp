...
void sortowanieHybrydowe(int* tab, int n) {
	if (n <= 100) {
		...
		sortowanieGrzebieniowe(tab, n);
	}
}

//5.sortowanie w miejscu (in-place sorting), np. selection sort
...
int main() {
	...
	//=================LAB 7===================//
	//	//4.SORTOWANIE HYBRYDOWE
	cout << "Sortowanie hybrydowe:\t ";
	start = clock();
	sortowanieHybrydowe(tab_posortowana, n);
	stop = clock();
	czas = (long double)(stop - start) / CLOCKS_PER_SEC;
	cout << "\tczas: " << czas << " s\n";
	...
	//5.SORTOWANIE W MIEJSCU - np. sortowanie przez selekcje 
	// /(np. sortowanie przez wstawianie - algorytm zewnetrzny)
	cout << "Sortowanie w miejscu (przez selekcje):\t ";
	start = clock();
	sortowaniePrzezSelekcje(tab_posortowana, n);
	stop = clock();
	czas = (long double)(stop - start) / CLOCKS_PER_SEC;
